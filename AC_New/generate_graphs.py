...
performance_new=[0 for x in range(run_len)]
...
for run in range(num_runs):
	...
	for i in range(run_len):
		...
		performance_new[i]+=avg
		
	print('run_number:', run)
performance_old=[x/num_runs for x in performance_old]
performance_new=[x/num_runs for x in performance_new]
aa=nx.from_numpy_array(np.array(pop.adj_matrix))
nx.draw(aa)
plt.show()
plt.plot(performance_old,label='old_'+str(num_runs)+'_'+str(run_len))
plt.plot(performance_new,label='new_'+str(num_runs)+'_'+str(run_len)+'_'+str(.7))
